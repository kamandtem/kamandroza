...
export const IntroSlides: React.FC<IntroSlidesProps> = ({ onDone }) => {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#fffaf1] via-[#fffdfa] to-[#f7f0ff] flex items-center justify-center px-5 py-8">
      <section className="w-full max-w-md space-y-5 text-right">
        ...
        <div className="rounded-[2rem] bg-[#fffdf9] border border-[#f0e5d6] p-5 shadow-[0_18px_50px_rgba(82,62,45,.1)]">
          <div className="h-[280px] rounded-[1.5rem] bg-[#fff8ed] flex items-center justify-center overflow-hidden"><img src={slide.image} alt="تصویرسازی معرفی رزا" className="h-full w-full object-contain" /></div>
          <div className="pt-5 space-y-2"><h1 className="text-xl font-black text-[#20334d] leading-8">{slide.title}</h1><p className="text-sm leading-7 text-slate-600">{slide.text}</p></div>
        </div>
