...
export const NumberStepper: React.FC<NumberStepperProps> = ({
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-1.5">
        <button
          type="button"
          onClick={() => onChange(clamp(value - step))}
          disabled={value <= min}
          className="icon-only w-10 h-10 rounded-xl bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center text-slate-600 dark:text-slate-200 disabled:opacity-30 active:scale-95 transition-transform shrink-0"
